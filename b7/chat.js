...
document.addEventListener('DOMContentLoaded', () => {
   ...
   function showTypingIndicator(chatBox) {
    ...
    // Remove any existing indicator first (avoid duplicates)
    hideTypingIndicator();
...
async function handleEveryWord(messageText, commands = {}) {
    ...
    // ✅ Import extern module once (for efficiency)
...
try {
    ...
    if (externModule.default && typeof externModule.default === 'function') {
        ...
        console.log(" externHandler loaded successfully.");
    } else {
        console.warn(" extern.js found but no default function export.");
    }
} catch (err) {
    console.warn(" No extern handler found at ./extern/extern.js");
}

// Later, when you send words to externHandler:
if (externHandler) {
    try {
        console.log(`Sending to externHandler: "${words}"`);  // <-- print the word
        await externHandler(words);
    } catch (e) {
        console.error(`Extern handler failed for word "${words}":`, e);
    }
}
    ...
    for (let i = 0; i < words.length; i++) {
        ...
        if (commands[cleanedWord]) {
            ...
            // 🚀 Always send to extern handler too
            if (externHandler) {
                try { await externHandler(cleanedWord); } 
                catch (e) { console.error(`Extern handler failed for word "${cleanedWord}":`, e); }
            }
            continue;
        }

        // 2️⃣ Check for number + possible module (e.g. "5 math")
        ...
        if (!isNaN(numValue) && i + 1 < words.length) {
            ...
            try {
                ...
                if (module.default && typeof module.default === 'function') {
                    ...
                    i++; // skip the next word since it was used
                }
            } catch (err) {
        ...
        // 3️⃣ Try dynamic folder/module by word
        try {
        ...
        // 🚀 Always send to extern handler at the end of each word
        if (externHandler) {
            try { await externHandler(cleanedWord); } 
            catch (e) { console.error(`Extern handler failed for word "${cleanedWord}":`, e); }
        }
